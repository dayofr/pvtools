...
export default function DropzonePvgis({setPvgis, setNameFile}) {
    ...
    const onDrop = useCallback((acceptedFiles) => {
        acceptedFiles.forEach((file: File) => {
            ...
            reader.onload = () => {
                ...
                const sliced: [] = parsed.data.slice(11);
                ...
                for (let m in res) {
                    json.data[m] = {};
                    for (let d in res[m]) {
                        json.data[m][d] = res[m][d].reduce((curr: number, b: Datapoint) => (curr + b.production), 0) / res[m][d].length;
                    }
                }
                setPvgis(json)
            }
